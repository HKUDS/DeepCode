...
**[2026-07-08] 循环工程:给它一个目标,它自己跑到测试通过**

- **自主、测试驱动的循环。** `python -m cli.loop_cli "实现 X 并带测试" -w ./proj --test-cmd "python -m pytest -q"` 让 agent 自主把目标做完:每轮干活后循环**真跑你的测试**(绝不信模型口头之言),失败就把真实报错喂进下一轮,直到通过。失败棘轮在某个修法不奏效时逼它换思路;熔断在测试变绿、轮次耗尽或停滞时收手。进度以 JSON 落在 `.deepcode/loop/state.json`,每轮一个影子 git 快照。
- **会自我整理的记忆。** 后台整理会合并重复笔记、删除陈旧内容、把 `MEMORY.md` 保持为精简索引——让 agent 的记忆在多会话中始终清晰。
- **可定时运行。** `python -m cli.schedule_cli` 按间隔运行循环或记忆整理,带 keepalive 门控:只在活儿没干完、上次干净退出、且未超运行上限时自唤醒续跑——不早死、不失控。
