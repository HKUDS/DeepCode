...
class DAGExecutor:
    ...
    async def _run_node(self, node: DAGNode) -> Any:
        ...
        优先级:
          1. 2D 矩阵查找 (MatrixToolRegistry.lookup)
          2. TransportAwareRegistry.execute() (位置透明)
          3. HandleFactory 全局兜底 (InProcess → Agent → MCP → Remote)
          4. 兜底
        """
        # 1. 尝试 2D 矩阵查找 (MatrixToolRegistry)
        if hasattr(self.registry, 'lookup'):
        ...
        # 2. TransportAwareRegistry.execute() (适用于任何 ToolRegistry)
        if hasattr(self.registry, 'execute'):
            try:
                call_result = await self.registry.execute(node.tool_name, node.args)
                if call_result.success:
                    return call_result.result
                node.error = call_result.error
            except Exception as e:
                node.error = str(e)

        # 3. HandleFactory 全局兜底
        if not node.error:
            try:
                from transparent_handle import call as transparent_call
                result = await transparent_call(node.tool_name, node.args)
                if result.success:
                    return result.data
