        ...
        *,
        ...
        for definition in sorted(definitions, key=lambda item: str(item.name)):
            ...
            if not server.definition.exposes(raw_name):
                continue
            # P1-9 (lesson 13): supply-chain allowlist — a server that is
            # not on DEEPCODE_MCP_SERVER_ALLOWLIST registers no tools (remote
            # MCP is the widest third-party surface).
